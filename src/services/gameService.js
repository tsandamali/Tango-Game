...
class GameService {
  ...
  static submitSolution(gameId, playerId, solution, round) {
    ...
    const validation = this.validateTangoSolution(solution, roundData.puzzle);
    if (!validation.valid) {
      return { success: false, error: 'Incorrect solution', validationError: validation.reason, validationErrors: validation.errors };
    }
  ...
  static validateTangoSolution(userSolution, puzzle) {
    const result = { valid: true, reason: '', errors: [] };
    if (!userSolution || !puzzle) {
      result.valid = false; result.reason = 'Missing solution or puzzle'; return result;
    }

    const size = puzzle.size;
    if (!Array.isArray(userSolution) || userSolution.length !== size) {
      result.valid = false; result.reason = 'Invalid solution shape'; return result;
    }

    // Check cells values and collect empties
    for (let r = 0; r < size; r++) {
      if (!Array.isArray(userSolution[r]) || userSolution[r].length !== size) {
        result.valid = false; result.reason = 'Invalid solution row shape'; return result;
      }
      for (let c = 0; c < size; c++) {
        const v = userSolution[r][c];
        if (v !== 1 && v !== 2) {
          result.valid = false;
          result.reason = 'Invalid cell values (must be 1 or 2)';
          result.errors.push({ row: r, col: c, reason: 'Invalid value' });
        }
      }
    }
    ...
    // Rows: counts and three-in-row
    for (let r = 0; r < size; r++) {
      let sun = 0, moon = 0;
      for (let c = 0; c < size; c++) {
        const v = userSolution[r][c];
        if (v === 1) sun++; else moon++;
      }
      if (sun !== half || moon !== half) {
        result.valid = false;
        result.reason = `Row ${r} has wrong counts`;
        for (let c = 0; c < size; c++) result.errors.push({ row: r, col: c, reason: 'Row count mismatch' });
      }
      // check horizontal triples
      for (let c = 0; c <= size - 3; c++) {
        if (userSolution[r][c] === userSolution[r][c+1] && userSolution[r][c] === userSolution[r][c+2]) {
          result.valid = false;
          result.reason = `Row ${r} has three-in-a-row at cols ${c}-${c+2}`;
          result.errors.push({ row: r, col: c, reason: 'Three-in-row' });
          result.errors.push({ row: r, col: c+1, reason: 'Three-in-row' });
          result.errors.push({ row: r, col: c+2, reason: 'Three-in-row' });
        }
      }
    }
    if (!result.valid) return result;

    // Columns: counts and three-in-row
    for (let c = 0; c < size; c++) {
      let sun = 0, moon = 0;
      for (let r = 0; r < size; r++) {
        const v = userSolution[r][c];
      ...
      if (sun !== half || moon !== half) {
        result.valid = false;
        result.reason = `Column ${c} has wrong counts`;
        for (let r = 0; r < size; r++) result.errors.push({ row: r, col: c, reason: 'Column count mismatch' });
      }
      for (let r = 0; r <= size - 3; r++) {
        if (userSolution[r][c] === userSolution[r+1][c] && userSolution[r][c] === userSolution[r+2][c]) {
          result.valid = false;
          result.reason = `Column ${c} has three-in-a-row at rows ${r}-${r+2}`;
          result.errors.push({ row: r, col: c, reason: 'Three-in-row' });
          result.errors.push({ row: r+1, col: c, reason: 'Three-in-row' });
          result.errors.push({ row: r+2, col: c, reason: 'Three-in-row' });
        }
      }
    }
    if (!result.valid) return result;
    ...
    if (constraints.horizontal) {
      for (const constraint of constraints.horizontal) {
        ...
        if (constraint.type === 'equal' && left !== right) {
          result.valid = false; result.reason = 'Horizontal constraint violated';
          result.errors.push({ row: constraint.row, col: constraint.col, reason: 'Constraint equal violated' });
          result.errors.push({ row: constraint.row, col: constraint.col+1, reason: 'Constraint equal violated' });
        }
        if (constraint.type === 'different' && left === right) {
          result.valid = false; result.reason = 'Horizontal constraint violated';
          result.errors.push({ row: constraint.row, col: constraint.col, reason: 'Constraint different violated' });
          result.errors.push({ row: constraint.row, col: constraint.col+1, reason: 'Constraint different violated' });
        }
      }
    }
    if (!result.valid) return result;
    ...
    if (constraints.vertical) {
      for (const constraint of constraints.vertical) {
        ...
        if (constraint.type === 'equal' && top !== bottom) {
          result.valid = false; result.reason = 'Vertical constraint violated';
          result.errors.push({ row: constraint.row, col: constraint.col, reason: 'Constraint equal violated' });
          result.errors.push({ row: constraint.row+1, col: constraint.col, reason: 'Constraint equal violated' });
        }
        if (constraint.type === 'different' && top === bottom) {
          result.valid = false; result.reason = 'Vertical constraint violated';
          result.errors.push({ row: constraint.row, col: constraint.col, reason: 'Constraint different violated' });
          result.errors.push({ row: constraint.row+1, col: constraint.col, reason: 'Constraint different violated' });
        }
      }
    }
